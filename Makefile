...
.PHONY: dc-build
dc-build: dc-down
	docker-compose -p ozon_route256 up -d --no-deps --build --force-recreate act-device-api
...
check-migrations:
	goose -dir migrations postgres "postgres://docker:docker@localhost:5432/act_device_api?sslmode=disable" status
